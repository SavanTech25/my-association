# Variables
PROJECT_NAME = Mon Asso
# Node.js version to install
NODE_VERSION = 20  # Change this to the version you need
...
firebase-deploy:
	@echo "$(ccso)--> Deploying to Firebase Hosting... $(ccend)"
	\. "$(NVM_DIR)/nvm.sh" && nvm use 20 && \
	npm run build && \
	npm install -g firebase-tools && \
	firebase deploy --only hosting:my-association